...
export default function LandingPageHeader(): ReactElement {
  ...
  const setAppTheme = (): void => {
    if (theme.palette.mode === 'light') theme.palette.mode = 'dark'
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!matches && (
          <Typography variant="inherit" sx={{ letterSpacing: '2px', mt: 1 }}>
            UNetwork
          </Typography>
        )}
      </Box>
      {!matches && (
        <Box display="flex" width={!matches ? 300 : 100} gap={5} p={1}>
          <StyledLink
            ...
            name={t(I18N.LANDING_PAGE.NAV.BTN1)}
          />
      ...
      <Box gap={2} display="flex" height="100%">
        <Box mr={2}>
          ...
          >
            {t(I18N.LANDING_PAGE.NAV.BTN5)}
          </Button>
          <Button
            onClick={() => {
              router.push('/auth/register')
            }}
            sx={{ borderRadius: '20px', marginLeft: '25px' }}
            variant="contained"
          >
            {t(I18N.LANDING_PAGE.NAV.BTN6)}
          </Button>
        </Box>
        {/* <Box height={40} border='1px solid'></Box> */}
        <Divider
          sx={{ borderColor: 'tinyElements' }}
        ...
        <Box display="flex" justifyContent="center" alignItems="center">
          {theme?.palette.mode === 'light' ? (
            <DarkMode
              onClick={setAppTheme}
              sx={{ height: '30px', width: '50px', cursor: 'pointer' }}
            />
          ) : (
            <LightMode
              onClick={setAppTheme}
