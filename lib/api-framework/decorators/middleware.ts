  ...
  try {
    ...
    try {
      ...
      if (!jwtSecret) {
        return {
          valido: false,
          erro: 'Configuração de segurança ausente no servidor'
        }
      }
...
export function JwtAuth(roles?: string[]) {
  return function (
    ...
    const jwtMiddleware: MiddlewareFunction = async (
      requestData: ControllerMethodParams
    ) => {
      ...
      if (roles && roles.length > 0) {
        ...
        if (!temPermissao) {
          return NextResponse.json(
            {
              error:
                'Acesso proibido. Você não tem permissão para este recurso.'
            },
            {
              status: 403
            }
    ...
    try {
      const existingMiddlewares: MiddlewareFunction[] =
        Reflect.getOwnMetadata(MIDDLEWARE_KEY, target, propertyKey) || []
...
export const middlewareRegistry = new Map<
  any,
  Map<string, MiddlewareFunction[]>
>()

// Função para registrar um middleware sem usar reflect-metadata
export function registrarMiddleware(
  target: any,
  methodName: string,
