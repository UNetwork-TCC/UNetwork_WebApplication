...
export function Body() {
  return function (target: any, propertyKey: string, parameterIndex: number) {
    const existing: Record<number, boolean> =
      Reflect.getMetadata('body', target, propertyKey) || {}
...
export function Param(paramName?: string) {
  return function (target: any, propertyKey: string, parameterIndex: number) {
    const existing: Record<number, string | true> =
      Reflect.getMetadata('param', target, propertyKey) || {}
...
function getArgumentsFromRequest(
  ...
  for (let i = 0; i < paramTypes.length; i++) {
    ...
    const bodyMetadata =
      Reflect.getMetadata('body', target, methodName) ||
      (prototype
        ? Reflect.getMetadata('body', prototype, methodName)
        : undefined) ||
      {}
    if (bodyMetadata[i]) {
    ...
    const paramMetadata =
      Reflect.getMetadata('param', target, methodName) ||
      (prototype
        ? Reflect.getMetadata('param', prototype, methodName)
        : undefined) ||
      {}
    if (paramMetadata[i]) {
...
export function Controller<T extends new (...args: any[]) => any>(
  ...
  const DELETE_HANDLER = createNextRouteHandler(
    controllerClass,
    'DELETE',
    'Delete'
  )
  const PATCH_HANDLER = createNextRouteHandler(
    controllerClass,
    'PATCH',
    'Patch'
  )
  ...
  return async function (request: NextRequest, context?: any) {
    ...
    if (!controllerInstances.has(controllerClass)) {
      try {
            ...
            try {
              ...
              try {
                ...
                if (paramName.endsWith('Repository')) {
                  ...
                  const repoClass = require(
                    `@/lib/server/repositories/${repoName}`
                  )[repoName]
                  if (repoClass) {
                ...
                console.warn(
                  `Não foi possível resolver dependência: ${paramName}`,
                  e
                )
              }
              return undefined
        ...
        console.warn(
          'Erro ao criar controlador com injeção de dependência:',
          error
        )
        const instance = new controllerClass()
        controllerInstances.set(controllerClass, instance)
    ...
    const methods = Object.getOwnPropertyNames(controllerClass.prototype)
      ...
      .filter(prop => {
        ...
        return (
          descriptor &&
          descriptor.value &&
          descriptor.value[`__is${handlerType}Handler`]
        )
      })
    ...
    if (methods.length > 0) {
      ...
      try {
        ...
        const middlewareResponse = await executarMiddlewares(
          instance,
          lastMethod,
          requestData
        )
        ...
        const hasPromises = args.some(
          arg => arg && typeof arg.then === 'function'
        )
