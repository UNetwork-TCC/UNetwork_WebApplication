...
export class BaseRepository<T> {
  ...
  async fetchAll(
    query?:
      | FilterQuery<any>
      | (QueryOptions & { populate?: any; select?: any; lean?: boolean }),
    options: QueryOptions & { populate?: any; select?: any; lean?: boolean } = {
      limit: 20
    }
  ) {
    ...
    if (query && typeof query === 'object' && !Array.isArray(query)) {
      ...
      for (const [k, v] of Object.entries(query)) {
        if (knownOptionKeys.has(k)) maybeOptions[k] = v
        else maybeFilter[k] = v
      }
    ...
    if (finalOptions.populate)
      queryBuilder = queryBuilder.populate(finalOptions.populate as any)
    if (finalOptions.select)
      queryBuilder = queryBuilder.select(finalOptions.select as any)
    if (finalOptions.lean) queryBuilder = queryBuilder.lean()
    return (await queryBuilder) as unknown as ResponseDTO<T[]>
  }
  async findById(id: string) {
    return (await this.entity.findById(id)) as unknown as ResponseDTO<T>
  }
  async create<K = Partial<T>>(data: K) {
    return (await this.entity.create(data)) as unknown as ResponseDTO<T>
  }
  async update(id: string, data: UpdateQuery<T> | Partial<T>) {
    return (await this.entity.findByIdAndUpdate(
      id,
      data
    )) as unknown as ResponseDTO<T>
  }
  async delete(id: string) {
    return (await this.entity.findByIdAndDelete(
      id
    )) as unknown as ResponseDTO<T>
  }
}
